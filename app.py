...
#            KHA TEFAS fon bilgi kartı (kullanıcı tarafından paylaşıldı)
FON_DETAY_BILGI = {
    ...
    "KHA": {"alis_valoru": "T+1", "satis_valoru": "T+2", "risk_degeri": "6/7", "son_emir_saati": "13:30"},
}
# ── Fon İçerik Analizi ────────────────────────────────────────────────────────
...
def piyasalar_debug():
    ...
    try:
        ...
        bilgi["altin_s1_doviz"] = fetch_altin_s1_doviz()
    except Exception as e:
...
def fon_icerik_getiri_debug():
    ...
    (1 ay/3 ay/6 ay/1 yıl) neden boş/başarısız döndüğünü tek tek görmek için.
    CRON_KEY ile korunur (login gerektirmez, ben de kontrol edebileyim diye)."""
    key = request.args.get("key", "")
    if key != os.environ.get("CRON_KEY", ""):
        return "yetkisiz", 403
    ...
    def ham_sorgu(hedef_tarih, pencere_gun=4):
        bas = hedef_tarih - timedelta(days=pencere_gun)
        bit = min(hedef_tarih + timedelta(days=pencere_gun), bugun)
        body = {
            "fonTipi": "YAT", "fonKodu": fon_kodu,
            "aramaMetni": fon_kodu, "fonTurKod": None,
            "fonGrubu": None, "sfonTurKod": None,
        ...
        try:
            ...
            return {
                "aralik": f"{bas} - {bit}",
                "http_status": r.status_code,
                "result_list_uzunluk": len(r.json().get("resultList", [])) if r.status_code == 200 else None,
                "yanit_ilk_300": r.text[:300],
            }
        except Exception as e:
            return {"aralik": f"{bas} - {bit}", "istek_hata": str(e)}

    noktalar = {
        "bugun": bugun,
        "getiri_1ay_-30gun": bugun - timedelta(days=30),
        "getiri_3ay_-90gun": bugun - timedelta(days=90),
        "getiri_6ay_-180gun": bugun - timedelta(days=180),
        "getiri_1yil_-365gun": bugun - timedelta(days=365),
    }
    pencere_sonuclari = {etiket: ham_sorgu(tarih) for etiket, tarih in noktalar.items()}

    # DB'deki mevcut cache durumu da görünsün
    with get_db() as conn:
        cache_satiri = conn.execute(
            "SELECT * FROM fon_getiri_cache WHERE fon_kod = ?", (fon_kodu,)
        ).fetchone()

    return jsonify({
        "fon_kodu": fon_kodu,
        "bugun": str(bugun),
        "pencere_sonuclari": pencere_sonuclari,
        "db_cache_kaydi": dict(cache_satiri) if cache_satiri else None,
    })
