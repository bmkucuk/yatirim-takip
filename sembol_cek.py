...
def cek_bist_hisseleri():
    """uzmanpara.milliyet.com.tr'den tüm BIST hisselerini çeker."""
    try:
        ...
        hisseler = []
        gercek_dis = {"Değişim","Adet","Hacim","Kod","Hisse","Sembol","Fiyat"}
        for row in soup.select("table tr"):
            cols = row.select("td")
            if not cols:
                continue
            kod = cols[0].get_text(strip=True)
            ad = cols[1].get_text(strip=True) if len(cols) > 1 else ""
            # Sadece gerçek hisse kodları: 2-6 karakter, büyük harf+rakam, özel kelime değil
            if (kod and 2 <= len(kod) <= 6 and 
                kod[0].isupper() and 
                kod not in gercek_dis and
                ":" not in kod and
                "%" not in kod):
                # İsim temizle
                ad = ad.replace("\xa0","").strip() if ad else ""
                hisseler.append((kod, ad, "HISSE", "BIST"))
        print(f"BIST: {len(hisseler)} hisse")
        return hisseler
    except Exception as e:
        print(f"BIST hata: {e}")
        ...
        return [
            ...
            ("MGROS","Migros"),("TCELL","Turkcell"),("TUPRS","Tüpraş"),
            ("TOASO","Tofaş Oto"),("FROTO","Ford Otosan"),("ASELS","Aselsan"),
            ("PGSUS","Pegasus"),("TAVHL","TAV Havalimanları"),("SASA","SASA"),
            ("EKGYO","Emlak Konut GYO"),("TTKOM","Türk Telekom"),("ARCLK","Arçelik"),
...
def cek_abd_etfleri():
    ...
    etfler = [
        ...
        # Uzay, savunma, teknoloji temalı ETF'ler
        ("SPCX","The Space ETF"), ("UFO","Procure Space ETF"), ("ROKT","SPDR S&P Kensho Final Frontiers ETF"),
        ("ITA","iShares U.S. Aerospace & Defense ETF"), ("XAR","SPDR S&P Aerospace & Defense ETF"),
        ("PPA","Invesco Aerospace & Defense ETF"), ("DFEN","Direxion Daily Aerospace & Defense Bull 3X"),
        # Kripto ETF'ler
        ("GBTC","Grayscale Bitcoin Trust"), ("ETHE","Grayscale Ethereum Trust"),
        ("BITB","Bitwise Bitcoin ETF"), ("EZBC","Franklin Bitcoin ETF"),
        # Diğer popüler
        ...
        ("NUSI","Nationwide Risk-Managed Income ETF"), ("SVOL","Simplify Volatility Premium ETF"),
        ("TNA","Direxion Small Cap Bull 3X"), ("TZA","Direxion Small Cap Bear 3X"),
        ("UVXY","ProShares Ultra VIX Short-Term Futures ETF"), ("VXX","iPath Series B S&P 500 VIX ETF"),
        ("BOIL","ProShares Ultra Bloomberg Natural Gas"), ("KOLD","ProShares UltraShort Bloomberg Natural Gas"),
        ("DUST","Direxion Daily Gold Miners Bear 2X"), ("NUGT","Direxion Daily Gold Miners Bull 2X"),
